...

{
    $id_no= $_POST['id_no'];
    $id= $_POST['id'];
    $product_name= $_POST['employee_name'];
    $description= $_POST['position'];
    $quantity= $_POST['salary'];
    $price= $_POST['hire_date'];

    $sql = "UPDATE employee SET id='$id', employee_name='$employee_name', position='$position', salary='$salary', hire_date='$hire_date' WHERE id_no =$id_no ";
...
<body>
    ...
    <input name="employee_name" type="text" id="b" value="<?= $record['product_name']; ?>" placeholder="Enter products name" class="w-full border border-blue-700 p-2 rounded-md">
    ...
    <input name="position" type="number" id="e" value="<?= $record['quantity']; ?>" placeholder="Quantity of the product" class="w-full border border-blue-700 p-2 rounded-md">

    <label for="d" class="p-2 text-xl">Price</label>
    <input name="salary" type="number" id="d" value="<?= $record['price']; ?>" placeholder="Enter the Price" class="w-full border border-blue-700 p-2 rounded-md">
